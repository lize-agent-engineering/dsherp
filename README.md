...
首次技术验证完成：固定 DSH SDK/Runtime `0.1.1rc1`；DeepSeek 官方最小调用及真实模型→DSH→MCP→ERPNext 只读闭环均已通过。独立 ERPNext `15.119.3` / Frappe `15.118.0` 站点已运行，普通用户读取、权限拒绝已验证。41 项自动化测试通过（其中工具链测试使用本地模型替身，付费真实闭环单独验证）。已完成本地原生初始化，验证中文 Workspace、物料录入搜索和管理员角色页；合成销售订单明细已保存回读、普通角色 UI 拒绝已验证（自动化数字输入使用标准 change 事件补充），亦无 SaaS 身份系统、制造业务链或生产部署。

阶段 1 样板验收完成，当前开始执行新计划阶段 2；[原生 Desk 证据与剩余事项](docs/engineering/native-desk-evidence.md)。本地入口：[原生登录页](http://127.0.0.1:18082/login)。

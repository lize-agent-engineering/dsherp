...
首次技术验证完成：固定 DSH SDK/Runtime `0.1.1rc1`；DeepSeek 官方最小调用及真实模型→DSH→MCP→ERPNext 只读闭环均已通过。独立 ERPNext `15.119.3` / Frappe `15.118.0` 站点已运行，普通用户读取、权限拒绝已验证。36 项自动化测试通过（其中工具链测试使用本地模型替身，付费真实闭环单独验证）。尚无 SaaS 身份系统、制造业务链、工作台或生产部署。

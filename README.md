...
阶段 1 样板与阶段 2 主流程原型已完成，阶段 3 已接入独立平台登录、成员绑定、自然语言任务与隔离 DSH 只读执行；完整容器链路已用本地模型替身验证，真实模型费用授权和页面端到端验收待完成；[原生 Desk 证据与剩余事项](docs/engineering/native-desk-evidence.md)。本地入口：[原生登录页](http://127.0.0.1:18082/login)、[应用构建原型](http://127.0.0.1:18082/app/dsherp-studio)。原型全部为合成数据，不代表真实生成、发布或迁移。

[真实 Agent 工作台实现与验收边界](docs/engineering/real-agent-workbench-evidence.md)。当前未启动付费查询服务，不能把自动化替身结果当成真实模型回答。

...
| 5 质量门禁（瘦身版） | CI 与每夜从零、一条命令开发栈、登记式清理、原生测试与权限矩阵、交接令牌落表 | 六个切片全部合入 main，PR 门与分支保护生效；**G9 起算点 2026-09-10 后移至 2026-09-09** （原定的 2026-09-08 当夜 `schedule` 那一次红在拆栈，PR #27 已修）——新起点是首个 `schedule` 触发的全绿夜 [run 34387910099](https://github.com/lize-agent-engineering/dsherp/actions/runs/34387910099)；30 天连续绿仍由 workflow 历史证明，**G9 未通过**；已裁决 #3/#4（当天所有 nightly 计入、夜按 Asia/Shanghai 贴标、起算点待 nightly 冻结后重钉），按该口径 09-09 与 09-10 都是红夜，起算点今天不钉，见终验台账 | [quality-gates-evidence](docs/engineering/quality-gates-evidence.md) |
| 6 Agent 质量与成本 | 工作流 F；注入信封 | 全部合入 main（PR #20–#34）；**G8 三条判据全部成立**（回放 35/35 = 100%、live 注入组 6/6 零提案、live 总体 31/34 = 91.2%；**终验台账更正**：91.2% 是重判读数，三次真批次为 23.5% / 67.6% / 76.5%，无一达 85%，且 live 无归档基线——三条并列今天不成立，待裁决 D15/D16），预算正式值按真实模型实测裁定；结束门以最终 main 上的全绿 nightly 为准：[run 34403951406](https://github.com/lize-agent-engineering/dsherp/actions/runs/34403951406)（`3de8e34`，19 步、集成 216、原生 146+5 且 0 跳过、回放 35/35）；合入前 49 代理审查抄出一条假绿与七条真缺陷，合入后 48 代理完成度自审补掉 18 项差，均已逐条修复 | [agent-quality-evidence](docs/engineering/agent-quality-evidence.md)、计划 [2026-09-08-agent-quality](docs/superpowers/plans/2026-09-08-agent-quality.md) |
| 终验 生产浸泡 | 全部 | 计划 7 本机切片：终验台账已建，十道门拆成 121 条判据逐条判定（33 成立 / 17 证据过期 / 10 无证据 / 61 未闭合，11 条阻断 G10）；G1 / G3 / G10 待主机与审计方，另有 21 项待裁决 | [final-acceptance-review](docs/engineering/final-acceptance-review.md) |
...
- [Agent 质量与成本证据（计划 6）](docs/engineering/agent-quality-evidence.md)与[评估集运行手册](evals/README.md)

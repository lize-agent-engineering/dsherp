...
当前隔离合成环境运行 ERPNext `16.33.0` / Frappe `16.31.0`，容器 Python `3.14.7`；固定 DSH SDK/Runtime 仍为 `0.1.1rc1`。当前最终门禁分别为非集成 `168 passed`、集成 `174 passed`、前端 21 个文件 `167 passed`、Node Runtime `8 passed`。自动化中的工具链使用本地模型替身，真实模型、真实 ERP 与浏览器证据另行记录，不能互相替代。

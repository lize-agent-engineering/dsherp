...
定位与注意：这是社区实践文章（安装、开发、运维、财务实战等），不是官方规范；成文时间和针对版本不一，参考做法前先核对与项目实测版本（Frappe `15.118.0` / ERPNext `15.119.3`）是否一致，涉及命令与 API 时以 GitHub tag 源码为准。访问规则与 V14 索引相同：URL 带尾斜杠，中文段按需百分号编码。

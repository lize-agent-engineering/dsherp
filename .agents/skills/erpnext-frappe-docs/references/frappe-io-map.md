...
- v15 维护线并行发版（Frappe `v15.119.1` 2026-08-27）：官方双线制——主线之外，各稳定版本线每周二例行发版（见发布流程）。项目安装的 v15 仍在维护期，未脱管。
- 项目锁定 Frappe `15.118.0` / ERPNext `15.119.3`：v16 的功能、字段、行为**不得**当作项目事实；升级是独立决策。
...
```bash
curl -s "https://api.github.com/repos/frappe/frappe/releases?per_page=10" | grep -o '"tag_name": "v15[^"]*"'
```

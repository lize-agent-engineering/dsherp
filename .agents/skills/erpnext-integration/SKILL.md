...
只操作 `dsherp-validation` 项目和合成站点。不复制 AgenERP 的卷、数据或凭证。当前已增加原生 nginx、WebSocket、worker 和 scheduler 进程，Desk 为本地 18082；已完成合成企业原生初始化和中文物料 UI 录入；调度已启用、普通用户实时认证已验证。明细草稿保存回读和普通角色 UI 拒绝已验证（数字输入使用标准 DOM change 事件补充），不等于完整企业业务建账。见 [原生 Desk 证据](../../../docs/engineering/native-desk-evidence.md)。

...
Two copies, byte for byte identical below this docstring — the Site's is
`frappe_app/dsherp_bridge/loop_guard.py`. The container has no Frappe and the Site has no
`dsherp` package, so neither can import the other's; a test asserts they have not drifted.
Same precedent as `usage.py` and `tool_limits.py`.
"""

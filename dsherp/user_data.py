...
# Per DocType: the columns that hold the person's own content, the facts that stay, and why.
BOUNDARY = {
    'DS Model Run': {
        'clear': ('question', 'page_context', 'answer', 'needs_input', 'error', 'platform_grant'),
...
def plan(*, user, conversations, runs, proposals, executions, sessions):
    ...
    return {
        ...
        'clears': {doctype: list(rules['clear']) for doctype, rules in BOUNDARY.items() if rules['clear']},

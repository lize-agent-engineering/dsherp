...
**同一天多个集只留最新的一个**——包括 `kind=release` 的升级前集：2026-09-15 一天里两次发布，第二次发布的升级前集在发布后
两分钟就被当天的常规备份顶掉、从异地淘汰了（本机 `archived/releases/<tag>/` 的那份不受影响，`rollback` 用的是它）。
要让升级前集在异地多留一段，是待裁决的策略，目前没有。淘汰先把集记成 `dropping` 再动两个仓库，一侧删完另一侧失败时下一轮接着删，
不会把它当成「异地不再完整」去重传。

...
- 配置及普通运行权限回归、来源交接撤权/重启及来源运行组合 6 项 / 28.45s。旧运行、旧配置交接在停用期间被拒，重新启用后也不能复用；需要重新提出。测试事务回滚，合成用户恢复启用。

## 最新接续：模型配置包进入侧栏

- 会话现返回当前用户可读的 configuration_bundles；配置包显示复用原确认卡的差异表，服务端提供具体字段、原生权限与工作流影响。用户可选“应用配置”领域。已有确认按 bundle_id 关联，恢复会话不会把同一包和确认重复显示。
- 隔离预览站点上“查看预览确认”仅持久化确认、不会执行 DDL；随后仍需明确点击“确认应用到隔离预览”。来源未成功不允许准备，错误不会自动重发；刷新只恢复记录。非预览站点目前明确显示尚需隔离预览，跨站动作尚未接通，不能当作完整用户路径。
- 配置包组件缺失、会话缺少包字段、API 路由/侧栏入口/配置领域选项缺失均先红测，再最小实现。80 项前端 / 5.49s 与构建通过；真实 beta 确认/来源/领域 3 项 / 20.68s；重启 alpha/beta 后真实固定 Runtime→MCP→HTTP 配置链 1 项 / 19.32s。模型仍是本地替身，本批未做新的真实浏览器或付费模型验收。
- 接续重点是固定源站/预览站之间只传不可变配置数据、按已验证用户对应身份接收、原生预览确认与结果回读，再在源站独立准备并确认发布。不得向模型开放站点/身份/凭据参数，不得以共享管理员执行业务。当前 worker 仍固定 alpha，beta 已有协议测试但尚无通用日常消费者；配置预览确认过期后的 UI 重准备也待补。其余阶段与完整目标继续 active。
...
# 配置边界与真实模型发布（2026-08-29）

- 配置确认过期后服务端在任何 DDL 和执行记录前拒绝，随后只允许用户重新准备一个新确认；两个并发、已认证 HTTP 确认返回同一个执行记录，原生配置只创建一次。没有新增发布角色或额外审批层。
- Partial、Unknown、Failed 结果可由当前用户只读核实每个冻结配置对象的当前原生状态与版本；核实不会重试、改写执行结论或把“当前一致”冒充原请求确定成功。没有执行记录时不靠配置列表猜测。
- 非 Item、Customer、Sales Order 的 Desk 表单现在明确作为有限上下文发送，避免把 Custom Field 等配置元数据误当业务对象而触发错误的业务权限检查。

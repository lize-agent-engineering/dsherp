...
每夜工作流每次从零、只跑一轮，1 GiB 有约 40% 余量，因此**不改上限**——改了会掩盖增长，
也在没有证据的情况下改动环境契约。风险如实记：每夜多了 `up --provision`（四站建站的大量 DDL）
在同一个数据库生命周期内，首夜的 `work/docker-stats.txt` 是这一项的第一份实测；
长期运行的本机开发栈需要定期 `down`/`up`。

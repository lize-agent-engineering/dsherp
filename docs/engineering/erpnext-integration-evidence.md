...
| Work Order → Material Transfer for Manufacture Stock Entry | `erpnext.manufacturing.doctype.work_order.work_order.make_stock_entry` | `(work_order_id: str, purpose: str, qty: float \| None = None, target_warehouse: str \| None = None, source_stock_entry: str \| None = None)` | 调用值：`purpose="Material Transfer for Manufacture"`；安装源码 `work_order.js:734,966-975` 传入该字面量，`work_order.py:1578-1630` 接收并处理它 |
| Work Order → Manufacture Stock Entry | 同一 `make_stock_entry` | 同上 | 调用值：`purpose="Manufacture"`；安装源码 `work_order.js:779,794,803,966-975` 传入该字面量，`work_order.py:1068-1070` 将两种 purpose 作为 Work Order Stock Entry 值处理 |

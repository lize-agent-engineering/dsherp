...
| 用量计量（T5） | 运行按服务端自己收到的 `model_response` usage 结算，未知不记 0 而是记进 `usage_unknown_calls`；六个计量字段与回填 patch；宿主 `usage <月份>` 按站点时区换算月份 |
...
`platform_grant`、会话标题），保留追责事实（谁、何时、读了哪些记录、提案与执行结果）。留下什么也

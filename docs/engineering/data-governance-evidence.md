...
- **`release <tag>`**（`dsherp/admin.py`）：核对 `prod.env` 的 tag → 对每个站（租户站与平台站）：无在飞运行才继续，写 `maintenance_mode`/`pause_scheduler` 静默 → `bench backup --with-files` → 四件套复制到 `archived/releases/<tag>/<站>/`（backend 的 `tenant-archive` 卷与新增的 platform-backend `platform-archive` 卷，躲开 Frappe 23 小时的自动清理）→ 升级前快照落 `.runtime/releases/<tag>/<站>/before.json` → `bench migrate` → 升级后快照 → 收集声明 → 比对 → 恢复站点标志。migrate 失败：站点保持维护模式，写带 `failed` 的部分报告，退出码 2。报告文件带时间戳不覆盖。
- **`rollback <tag>`**：参数是要撤销的发布；从记录找到归档备份集（`--backup SITE=FILE` 可覆盖），静默后 `bench restore <db> --with-public-files … --with-private-files … --force`（root 口令经脱敏），恢复后快照并与 `before.json` 比对，唯一容忍的是 restore 自己回写的 `System Settings.enable_scheduler`（单值文档的 `modified`/`modified_by` 一律不比）；有差异退出码 1。
- 新增 `snapshot <站> --out FILE` 与 `compare BEFORE AFTER` 两个命令供演练与排查。
...
| 门 | 结果 |
|---|---|
| 非集成 `pytest tests --ignore=tests/integration` | `424 passed`（main 合并后 402；本切片新增 22 条：比对 11、读取 8、CLI 8 替换原 5 条、契约 1 处扩展） |
| Node / 前端 | Node `10/10`；前端未改动 |
| 集成 | 本切片不改 Frappe App 代码，未重跑集成门 |

### 未闭合

- G2 按 spec 第 29 行仍待审计方在合规主机上执行；本节是执行方自报的本机演练。

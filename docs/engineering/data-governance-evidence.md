...
## 切片 3：审计保留、业务单据关联、用量计量与个人数据边界

裁决 #3 把"审计记录是对外承诺"写成代码级边界，裁决 #10 划定删除范围。本切片把这两条落到八个审计
DocType、执行记录与业务单据的关联、按站点时区的用量计量，以及一个人的数据能被导出与删除。

### 做了什么

| 项 | 结果 |
|---|---|
| 审计不可篡改（T2） | 八个审计 DocType 加 `track_changes`、去掉任何 delete/cancel 权限，控制器 `on_trash` 无条件拒绝；策略（`DS Doctype Policy`）仍可改，但每次保存必须给出**新的**原因；`DS Conversation` 在仍有运行记录引用时不可删 |
| schema 演进（D4） | `migrate-drill` 把旧构建的备份集恢复进跑新构建的隔离栈并 `bench migrate`，按 G2 口径判定未声明差异 |
| 用量计量（T5） | 运行按服务端自己收到的 `model_response` usage 结算，缺失或只报了一半的调用记进 `usage_unknown_calls`（已知的那一半仍累加）；六个计量字段与回填 patch；宿主 `usage <月份>` 把站点本地时间换算成 UTC 后归月，月报在站点与总计里带出 `unknown_calls` 与 `complete` |
| 读放大与索引（T8） | 会话列表改为按页取数并只授权这一页；`conversation`/`status`/`expires_at`/`queue_expires_at` 建索引 |
| 业务单据关联（T3） | 执行记录用 Dynamic Link 指向它产生的单据，单据侧可原生反查；单据被取消或删除时记录留下并标注，链接列入 `ignore_links_on_delete`，绝不挡用户处置自己的单据 |
| 会话存储纳管（T4） | 原生会话目录改为 `<站点>/<会话>/<scope>`，可按用户经其会话枚举；`sessions` 命令查看与按 90 天清理；旧的扁平哈希目录如实报为"未归属"而不猜测 |
| 个人数据导出与删除（T4） | `export-user-data` 只读导出为 0600 文件；`delete-user-data` 先导出再清除，`--confirm` 才执行，不带确认时打印计划并以退出码 1 结束 |

删除边界逐 DocType 声明在 `dsherp/user_data.py`：清除本人内容（提问、页面快照、回答、错误正文、
会话标题），保留追责事实（谁、何时、读了哪些记录、提案与执行结果）。平台授权令牌不在行里（R7 之后只在
运行期间存于缓存）。留下什么也
写明：`DS Run Event` 不改写、只能走登记的受控脱敏迁移；已生成的异地备份按保留策略到期淘汰，不为
一次删除请求改写历史。

### 真实链路（dev 栈 `dsherp-validation.localhost`）

| 场景 | 结果 |
|---|---|
| 审计标记落库 | `bench migrate` 在真实站点应用；62 条运行回填用量；29 条执行记录关联到它产生的单据 |
| 导出 | 读者账号 13 个会话 / 14 条运行导出为一份 0600 文件，提问与回答原文在内 |
| 删除计划 | 不带 `--confirm` 时打印将清除的列与保留项，退出码 1；不写站点 |
| 删除执行 | 提问、回答、页面快照、`platform_grant` 全部清空；写者账号 45 条提问原样不动；运行与会话行仍在，`sources` 与 `request_id` 保留；84 条事件未受影响；为验证而制造的那条含 `question` 的 Version 被删除，站点其余 1118 条版本不动；两个会话目录按会话名删除 |
| 清除后的可读性 | 第一版把 `page_context` 清成空串，真实站点上被删用户的旧会话直接 500——该列按站点自己的 schema 校验。改为清成一份合法的"未知页面"快照后复跑，18 条运行全部改写，站点无空值残留 |

### 未闭合与如实说明

- **`frappe.db.delete` 绕过 `on_trash`**：控制器的"无条件拒绝"是 Document API 路径上的无条件。数据库
  层删除仍然可行，集成测试的合成数据正是走这条路清理的。真正的不可删除需要数据库权限层面的约束，
  本轮不做，如实记录。集成测试的这条清理路径自计划 5 起由 `tests/integration/residue.py` 统一执行：
  测试在创建之前先把要留下的东西登记进 `.runtime/integration-residue.json`，teardown 由宿主按控制器
  要求的顺序清扫并核验零残留，清不掉的条目留在台账里等人处理，被中断的会话留下的登记在下一次会话
  开头清。持久夹具（`DSHERP-TEST-*` 等前缀）与固定演员邮箱在登记时就被拒绝，清扫碰不到它们。
- **删除不覆盖事件与历史备份**：见上文 residue；删除报告里逐条列出。
- **回收未做**：被清除的列不会自动收缩表空间，也不做重建；这属于运维动作。

## 切片 4：短期业务凭据与轮换（工作流 B 的 S2/S4/S9）

平台过去长期保存成员的业务站 `api_key`/`api_secret`，绑定在凭据就在。本切片改为借用有期限的凭据，
并给三类长期凭据一条轮换命令与一本账簿。窗口的拒绝只在 SSO 强制（`disable_user_pass_login`）的站点上
执行，与后台授权令牌的规则同一条线；开发站记录并报告，不拒绝。

### 并发登录的裁决（需用户复核）

Frappe 每个 `User` 只有一个 `api_secret`，所以短期凭据**属于业务用户，不属于某次会话**。据此定的规则：

1. 同一成员再登录时，凭据还在窗口内（剩余 > 4 小时）就原样交出，不重新签发——第二次登录不会踢掉
   第一次；只有进入续签窗口才换新。
2. `DS Membership` 拒绝把同一个业务用户绑给第二个启用中的平台成员，否则两个成员会轮流作废对方的
   凭据。
3. 成员绑定被停用时，平台请业务站吊销该凭据；送不到就如实说明，最长一个窗口后自动失效。
4. 窗口 12 小时、续签窗口 4 小时，写在 `dsherp/business_credentials.py`，两个 App 各存一份逐字镜像，
   测试比对，避免两侧对"过期"理解不一致。

如果要做到"每次登录一把、互不影响"，就必须绕开原生 `api_secret` 自建一套令牌校验；那是另一套认证
机制，本轮按设计里"用原生 `generate_keys`"的口径没有做。此处请用户裁决是否接受。

### 真实链路上暴露并修掉的四处

| 问题 | 后果 | 处理 |
|---|---|---|
| 过期凭据被拒返回 403 | 平台读作"这个成员没有权限"，登录本身也就修不好一个过期的凭据 | 改为 `AuthenticationError`（401），平台据此识别为"该续签了" |
| `desk_identity` 要求活凭据 | 凭据过期后连登录都做不了（登录正是续签的唯一入口） | 凭据过期时改为凭"上次证明"作答，但只在绑定仍指向同一个业务用户时才可以；`erp_user` 被改过就拒绝 |
| `desk_entry` 要求活凭据 | 成员被自己的过期凭据挡在续签入口之外 | 改为只校验绑定，不要求活凭据；业务站自己认证到达的人 |
| `binding_version` 用行的 `modified` | 凭据存在同一行上，每 12 小时续签一次会读成"绑定变了"，把该成员全部会话踢下线 | 改为按绑定字段取内容摘要 |

### 演练结果

| 场景 | 结果 |
|---|---|
| 登录取得凭据 | v11，业务站自查 200 |
| 越过窗口 | 业务站 401「业务凭据已过期，请重新通过企业平台登录」；平台读转 `CredentialStale` |
| 重新登录 | v12，业务站 200，平台记录同步（有效期、版本、已证明属于谁），平台读 200 |
| 临近失效再登录 | v12 → v13（进入续签窗口才换） |
| 窗口充裕再登录 | v13 → v13（同一成员再登录不互踢） |
| 迁移既有站点 | 两个 App 各一个 patch：既有 key 与既有绑定纳入窗口并记下"凭据已证明属于谁"；不这么做的话既有成员在窗口结束后连登录都做不了 |
| 运维命令 | `credentials <站点>` 列出借出的凭据与状态，有 key 却没有窗口的记为发现并以退出码 1 结束；`--issue` 为某业务用户重新签发并交给平台 |

轮换（`rotate`）：

| 场景 | 结果 |
|---|---|
| provider 同值 | 拒绝：「新值与当前值相同，这不是一次轮换」 |
| provider 换新 | 只有 `DEEPSEEK_API_KEY=` 那一行变，其余行逐字相同，权限仍是 0600；账簿记下前后指纹 |
| runtime 轮换 | 旧密钥 401、新密钥 200；worker 重启后心跳继续 |
| runtime 多 profile | 第一次带两个 profile 的轮换踩到"先换后写"：旧格式 profile 不认，密钥已换而文件没更新，运行中的 worker 只靠已加载的旧值活着。据此把文件校验提到签发之前并支持旧格式，再跑一次两个文件一致、worker 正常 |
| oauth-client 轮换 | 平台与业务站两侧更新后，`test_desk_sso` 8 项仍全绿（登录不受影响） |
| 账簿与 doctor | 三条记录只含类别、目标、版本、生效时间与指纹；doctor 只报唯一没轮换过的目标 |
| 会话不被轮换作废 | `runtime_revision` 不再计入 `DEEPSEEK_API_KEY`（宿主与 runner 两侧同步改），端点仍然计入 |

### 门禁

| 门 | 结果 |
|---|---|
| 非集成 pytest | `637 passed`（切片 2 结束时 533） |
| 集成（dev 栈） | `tests/integration/test_desk_sso.py` 8 项、`tests/integration/test_platform_identity.py` 14 项全绿 |
| 真实迁移 | 四个站点（validation / platform / beta / daily）各跑 `bench migrate` 应用本轮 patch |

### 未闭合与如实说明

- **并发登录的裁决**见上，等用户复核。
- **`rotate` 的命名约定**：命令按生产开通的约定找运行身份与 OAuth 客户端（站点 `site_config` 里的
  `dsherp_runtime_user`、`DSHERP <slug> Desk` 或回调地址）。dev 栈是另一套脚本开通的，命名不同，
  演练时靠"按站点声明的运行身份"和"按回调地址找客户端"这两条兜底路径完成，本身也是本轮为此加的。
- **未迁移站点是 fail-closed 的**：装了新代码但没跑 `bench migrate` 的站点，`DS Business Credential`
  不存在，机器凭据一律被拒。这是刻意的，但升级顺序必须是"先 migrate 再切流量"。
- **凭据交付走内网 HTTP**：dev 栈上平台与业务站之间是容器网络明文；生产按裁决 #8 由 TLS 承载。
- **吊销依赖平台可达**：停用绑定时若业务站不可达，凭据只能等窗口自然结束，报告里如实说明。

...
7. **R1-2026-09-01-T4.3-2**：新会话运行 `8941949…` 真实执行到 7 次调用、`20480` 预留输出后，第二次压缩预约被明确 HTTP 417 拒绝；仍为 0 提案、0 执行、0 采购单据。按实际调用形状先锁定第 8 次压缩后累计 `22528` 应放行，旧输出常量红、仅调输出常量后绿；随后锁定压缩后的第 9 次 conversation 必须有机会形成提案，8 次调用常量红。最终只对 **operation** 域设 9 次/`25600`，query/configuration 仍保持 8 次/`16384`，累计输入、各单次输出和容器资源不变；第 10 次仍明确失败。该次真实失败同样保留且不恢复，不以自动重试掩盖。
8. **R1-2026-09-01-T4.3-3**：第三个新会话运行 `050d7d49…` 在 8 次调用/`21504` 预留内先生成提案 `5he61l424p`，但随后的第二次原生压缩自身以 `summarization truncated at the token cap` 截断，整轮明确 Failed；失败运行提案不确认，采购业务单据仍为 0。operation 压缩 2048 的行为测试先红；只把 operation 压缩单次上限调为 3072 后，query 仍为 2048，原生压缩/拒绝/恢复与模型授权组合 10 项转绿。9 次/`25600` 累计上限恰能容纳“两次 3072 压缩 + 七次 3072 conversation”，第 10 次仍失败；configuration、query 和容器资源均不放松。
9. **R1-2026-09-01-T4.4**：首个委外 PO 创建运行 `450d3862…` 在 8 次/`24576` 已生成提案 `d3u0kkn4kt`，第 9 次必要压缩被累计输出 HTTP 417 拒绝，整轮 Failed；提案不确认，委外 ERP 单据仍为 0。实际复杂委外 schema/七个主数据读取需要“两次压缩 + 最终答复”，先写 10 次 operation 调用均放行、第 11 次明确失败的测试，旧 9 次常量红；只把 operation 调整为 10 次/`30720` 后绿。query/configuration 仍是 8 次/`16384`，单次 conversation/compaction、累计输入和容器资源不变；失败运行不恢复、不重放。
10. **T4.4 请求时限裁定**：新委外运行 `31a70310…` 在 9 次/`27648` 内生成提案时耗时超过 SDK 固定 90 秒，宿主明确 `TimeoutExpired`，运行 Failed、提案不可确认、ERP 零写入；容器与消费者均已退出。先写 operation 请求时限 120 秒、query/configuration/无域仍 90 秒的测试并确认 AttributeError 红，再最小接入 `open_runtime` 后运行器 20 项绿；外层容器 140 秒硬时限、模型/累计预算和资源均不变，不重试该运行。

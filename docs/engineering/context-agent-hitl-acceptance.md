...
- ruling 后的新会话运行 `8941949e2dc4bde3e1f6b4998d50d9175584b4a3f3d73a48dc3117719252ed54` 仍明确 Failed：7 次调用、`20480` 预留输出后，第 8 次必要压缩被预算 HTTP 417 拒绝；仍为 0 提案、0 执行和 0 采购单据。第二轮红绿先证明第 8 次压缩需达到累计 `22528`，再证明压缩后的第 9 次 conversation 会被旧 8 次上限拒绝。最终只把 operation 域收口为 9 次/`25600`，第 10 次仍失败；query/configuration 维持 8 次/`16384`，其他预算与容器资源不变。两次真实失败均未恢复或重放。
...
- 新委外运行 `31a703100e5b472ec92f378d3d61f00c4fb78c563443aba761f7dc66cdb39a83` 在 9 次/`27648` 内生成提案时超过 SDK 固定 90 秒，宿主明确 `TimeoutExpired`；运行 Failed、提案不可确认，ERP 仍零写入，容器与消费者均退出。先红后绿只把 operation 请求时限延至 120 秒，query/configuration 仍为 90 秒，外层 140 秒硬时限及所有模型/资源预算不变；运行器相关 **20 passed / 19.77s**，不恢复或重试该失败运行。
- 新会话 `grbdd53cll` 连续完成委外 PO 创建/提交、PO→SCO make、SCO 提交、供料 SE make/提交、SCR make/提交、fixture SO 提交、SO→DN make、DN 提交共 11 个真实模型运行。运行均 Succeeded，调用数 `10/4/3/3/5/3/4/4/3/5/3`，预留输出 `30720/12288/9216/9216/15360/9216/12288/12288/9216/15360/9216`；首轮恰到 10 次/`30720` ruling，未超过，第 11 次仍由测试明确拒绝。11 个提案 `hr9m1spem4` 至 `plsmu3l8ok` 各自且仅对应一条 Succeeded 执行，执行总数 11，无 Unknown/Failed/Running。

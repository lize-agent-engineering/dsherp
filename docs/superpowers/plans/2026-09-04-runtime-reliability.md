...
自检门规则：红灯先修；同一红灯修两次仍红即停下报告。

关于"计划未列文件"的停止阈值（2026-09-05 架构方修订）：判据不是文件数，而是**改动是否属于同一项已列变更的必要后果**。把一个常量从客户端搬到服务端，必然要动读它的每一处、算它指纹的地方和它的测试——这些是同一变更的连带路径，不触发停止，但**必须在该提交的证据里逐条列出**，不得只披露"核心"几个。真正触发停止的是**漂移**：改动进入与当前任务无因果关系的领域（例如为修超时而顺手重构提案链路），此时无论几个文件都要停。
...
## Deferred（明确不在计划 2 内关闭）

- `monitor_ops` 只覆盖 `sites[0]`：多站运维快照属计划 3 的部署规格；生产就绪审计的 R7 在本计划标"部分处理"。
- 关机排空上限 15s：单条运行最长 600s，重启时仍可能丢掉正在执行（尚未回写）的那条运行的答案。排空覆盖的是已拿到结果、只差 `finish_run` 的窗口。彻底解决需要可恢复的运行交接，属计划 3 的部署形态。
- 领取未确认的运行判 Failed 而不退回排队：服务端无法区分"worker 没收到响应"与"容器正在冷启动"，退回会带来两个执行者。用户需手动重发；`queue_expires_at` 仍是排队侧的兜底。

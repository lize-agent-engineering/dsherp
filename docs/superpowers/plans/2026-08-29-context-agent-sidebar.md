# 原生业务页面上的上下文 Agent

日期：2026-08-29。用户已确认执行；持续目标 active。当前 main 工作，不建 worktree、不推送。

## 目标与替代范围

本计划包含 Agent 执行循环、领域能力、上下文、会话、工具、人工确认及原生侧栏，不只是 UI。企业日常工作使用 Frappe/ERPNext 原生页面，全局按钮打开 React + Ant Design 右侧面板；不以 `/app/dsherp-home` 聊天页作为业务主入口，不做普通/Agent 两套模式。平台负责统一登录及企业成员关系；业务 Site 负责业务身份、权限、会话和执行记录。

本计划替代旧计划中的独立工作台、双空间导航、任意生成代码、破坏性结构修改方向。旧 Demo 保留独立入口、移出正式导航且不再扩展；其他被替代实现清理，有效回归保留。旧平台任务历史只读保留，不迁入新企业。

## 执行边界

- 先读入口、设计、计划、证据并复核代码/分支/运行；前端优先，每项行为先红再绿，fastfail，不默认新增门禁。
- 固定 SDK/Runtime 0.1.1rc1（源码 528c682e061696f5a160f363f236ecbf53cbd006）；不改上游核心，不另写 Agent 循环。SDK 未暴露的能力用项目 Cordis 插件接入，不伪称官方接口，不修改上游私有字段，不擅自升级或削减恢复/取消。
- 不修改 AgenERP/其他服务，不扫描其他项目密钥；本项目已有模型授权内使用，不能扩大生产写入或无限资源。仅缺凭证、新授权、关键架构决策询问。
- 不用定时自动化替代持续目标，全部目标实际达成才 complete。每阶段记录代码、单测、真实 Runtime、真实模型、真实 ERP、UI、部署，互不替代。
- 临时产出 work/，无后续用途清理；按功能本地提交，不提交密钥、租户数据、日志、运行状态。

## 插件、领域与协议

1. SDK 接入插件：显式创建、原生恢复、取消、安全关闭；复用 Agent、持久化、checkpoint 和协议。仅一个 stdio 入口，评估公开 JsonRpcLineTransport/HarnessSdkJsonRpcServer，不同时抢 stdin。
2. 业务上下文插件：页面快照、来源、版本、历史、业务状态及模型输入预算。
3. 查询、业务操作、应用配置三个领域各自限定工具和 skills；复用 MCP/Frappe。业务 skills 固定版本并校验摘要，只读指定目录、includeDefaultRoots=false、watch=false。缺失/错版报错，不加载个人开发 skills，不允许模型修改权限或工具集。首版无多 Agent 并发框架。
4. 工具/HITL 适配：模型生成提案，服务端确认后执行；业务、配置、填表都要 HITL。内部会话与审计持久化不属于待确认业务写入。

## 会话与上下文不变量

- 业务 Site 分别持久化用户会话、模型运行、操作提案、执行记录。每消息绑定发送时快照；跨页继续不改变已提出操作目标。
- 原生日志按企业/用户/会话/领域隔离，稳定 session_root/cwd，每运行会话单写者。跨进程必须显式原生 resume，不以相同 ID 或保留 Python wrapper 冒充恢复；退出确认 flush/dispose。
- 刷新只恢复展示，不自动运行模型/写入；request_id 去重。多标签并发必须协调。
- 等待 HITL 结束本轮运行并释放容器；确认重新检查身份、权限、版本并用新短期执行凭据。取消停止后续工作，不撤销既有写入；结果不明先核实，不盲目重试。
- 权限、运行配置或 skills 版本变化后废弃旧运行上下文，重新授权开启新运行；旧日志仅审计，不直接重喂模型。
- 页面快照覆盖路由、类型、DocType、记录、筛选、选中项及用户允许的未保存字段/子表变化。前端只是上下文，身份/权限/已保存事实由服务端确认。不整页抓 DOM、不默认上传全单/数据库/历史。
- 输入按需包括问题、快照、领域规则、近期对话、带来源摘要、待办提案及最新工具结果。保留调用-结果关系、来源及版本；完整提案存业务数据库，不依赖摘要重建。
- 历史和摘要重新使用前复查来源权限，撤权不能只隐藏 UI。所有模型调用（包括压缩直接 ctx.llm.stream）均检查权限和预算；阻止自动压缩 WARN AND CONTINUE 的不安全继续，不静默裁掉关键业务值、批准内容或版本。
- checkpoint 证明 Agent 状态，ERP 执行记录证明业务结果，二者不能替代。

## 阶段一：原生侧栏与真实多轮查询

- 原生业务 Site 登录下先交付面板，不等待统一登录：打开/关闭、跨页继续、新会话、历史、取消，显示使用的页面/单据，未知页面说明能力有限。
- 完成插件协议、跨进程持久化恢复、上下文管理、真实模型与当前用户可读数据。不得自动保存或刷新覆盖原生未保存内容。
- 随后平台到 Desk 登录衔接，校验成员关系和业务用户映射，使用 Site 验证身份，旧平台历史只读。

## 阶段二：业务操作与 HITL

- Item/Customer 新建和修改；Sales Order 草稿创建、修改、提交、取消；当前表单建议填入，确认后应用，保存和提交分别确认。
- 流程：需求 → 具体差异与影响 → 确认 → 服务端复查 → 原生业务方法 → 回读结果及记录。
- 确认绑定企业、操作者、动作、确切对象、内容和版本；批量冻结记录集合，不重新搜索扩大。业务确认默认十分钟，内容/权限/版本变化重确认。
- 服务端显式拒绝无字段权限的变更（Frappe 可能静默恢复字段），执行后核对实际差异；HTTP 成功不等于成功。原生手工按钮不叠加 Agent 确认。

## 阶段三：应用配置、预览与发布

- 生成原生配置实现字段、布局、保存及工作流，不生成任意代码。仅新应用和非破坏扩展，禁止删除/重命名既有字段、类型变化、隐式回填、覆盖既有 ERP 工作流。
- 沿用原生配置权限，不新增发布角色、不允许 Agent 提权。
- 隔离预览 Site 使用合成数据，禁止邮件/Webhook 等外部副作用；预览应用与目标发布分别确认。
- 发布绑定不可变配置包、目标及基线，默认三十分钟；自定义 App 钩子协调 Agent 与原生配置并发修改。
- 逐步记录结果；DDL 可能自提交，不承诺整体回滚；部分成功/结果不明停止核对，不整包重跑。

## 阶段四：日常使用及兼容

- 日常企业 Site 无合成数据，真实企业初始化由用户填写；首次真实数据使用前备份及隔离恢复验证。
- 沿用总约 4 GiB/2 CPU 授权，先核对常驻约 3712 MiB/1.9 CPU 与单 Agent 384 MiB/0.1 CPU；控制 Agent/预览并发。资源不足申请授权，不降低隔离、不默认加常驻服务。
- alpha/beta 保留合成验收；正式入口指向企业 Desk，平台首页不承担业务工作台。保留现有浏览器登录，不随意退出/关闭。

## 验收矩阵

- UI：列表/表单/子表/筛选/选中项；切页不串对象、不覆盖未保存内容。
- 会话：跨进程、多标签、刷新、断线、取消、压缩、配置版本变化；无重复执行/跨企业串话。
- 权限：只读禁止写、字段权限、撤权历史摘要、伪造上下文及工具请求。
- HITL：未确认零写；修改后重确认；过期/重复/并发/响应丢失/部分成功都有真实记录。
- 发布：隔离预览、原生保存工作流、并发配置、部分失败与结果核实。
- 真实固定版 DSH 插件、真实模型、真实 Frappe 权限和页面分别验证，替身不可替代。

## 现场进度与接续

- [x] 持续目标创建，main 干净，HEAD ffb7d52；已读入口/旧设计/当前计划/主要验证记录。
- [x] 确认本项目 11 个容器运行，旧付费 worker PID 39450 活跃；未停止任何服务，未运行写旧任务队列的测试。
- [x] 已确认计划落盘；旧方向标为被替代。
- [ ] 阶段一：先页面快照行为测试，再侧栏、业务会话、插件、真实验证及 SSO。
- [ ] 阶段二：业务 HITL。
- [ ] 阶段三：配置预览发布。
- [ ] 阶段四：日常 Site/备份恢复/兼容。

当前没有已确认外部阻塞。页面快照和侧栏交互首批 13 项 TDD 通过，全前端 48 项通过；尚未挂载 Desk/接新后端或真实恢复。不沿用旧链路证据声称完成。用户要求减少门禁和停止旧服务后，已核对无排队/运行任务并停止旧付费 worker PID 39450；未停原生 ERP/身份基础服务。证据持续记录在 docs/engineering/context-agent-evidence.md。

接续更新：同源客户端 3 项测试通过；固定 Runtime 插件 5 项真实进程协议测试通过（模型替身），包括显式跨进程 resume 和真实在途取消。全前端现 51 项、Python 根目录测试 35 项通过。用户已授权按计划继续使用项目原模型，无需重复询问。下一步与并发清理缺口见证据文件末尾；仍未完成业务会话后端、Desk 挂载、领域工具/skills、权限预算保护和真实模型/ERP/UI验收。

## 固定资料定位

最新接续：业务会话/运行、显式原生恢复/取消及跨进程单写者已建立；业务 run capability 经真实 Runtime→MCP→HTTP→ERP 读取通过（模型替身），相关组合 33 项通过。独立侧栏已通过 app_include_js 挂到 alpha 原生 Desk，前端 52 项通过；真实 UI 检查修复 navbar 遮挡并验证打开/关闭。现有 Administrator 登录未改变，普通用户真实模型 UI 尚未验收，新隔离执行器尚未消费队列。下一步运行账号、执行器、授权/版本/skills/预算，再真实多轮与 SSO；阶段一仍进行。详情以 context-agent-evidence.md 最后一节为准。

后续进展：已接入运行状态/取消联动，完成单次受限容器入口及两个独立容器显式原生恢复；项目真实 deepseek-v4-flash 在受限容器中读取合成 Item 并经服务端记录 Succeeded。常驻消费者、运行服务身份、权限与配置版本轮换及完整侧栏多轮仍未完成，继续按原阶段一推进，不缩减阶段二至四。

最新进展：已开通 alpha 专用无业务读权限的运行账号，消费者使用服务端 scope 目录、原生持久化查询后显式恢复，并与旧 worker 共用单并发锁。真实 HTTP 队列→两个独立业务运行/容器→结果落库回读通过（模型替身）。常驻进程尚未启动，下一步权限/配置/skills 轮换与全模型调用预算，之后普通用户真实模型 UI 和 SSO；原四阶段目标不变。

接续更新：模型流前授权/预算与查询领域原生权限版本已接入。真实角色变化停止旧运行并轮换原生会话目录；8项含实际队列容器的回归通过。仍需运行配置/skills 版本、真实压缩及普通用户侧栏多轮与SSO，再进入业务HITL/配置发布/日常站点，不将阶段一基础设施当完整交付。

当前更新：运行配置绑定/挂载文件漂移、erp-query 1.0.0 固定摘要与官方 skill loader 已接入，默认个人目录和 watcher 均关闭，真实原生 skill 正文加载及两轮容器队列回归通过（模型替身）。下一步原生压缩与来源保全、普通用户真实模型侧栏多轮/SSO，然后完成其余三阶段。

最新更新：原生自动压缩、拒绝摘要后禁止后续调用、压缩 checkpoint 跨进程显式恢复已验证（模型替身）；真实 ERP 读取的记录/列表/schema 版本进入来源账本，相关23项及Node5项通过。下一步普通用户真实模型侧栏多轮和SSO；真实模型摘要语义保全尚未验收，阶段二至四全部仍需实施。

真实 UI 更新：普通只读用户通过原生受审计身份切换，在 Item 表单和列表同一侧栏连续两轮真实 deepseek-v4-flash 查询成功；独立两容器恢复，数据库2条Succeeded，刷新后历史展示恢复。原管理员标签保留，无业务写入或常驻付费消费者。接续先处理必要侧栏体验/未保存与子表上下文及SSO，再业务HITL、配置预览发布和日常Site；完整验收缺口见最新证据节。

本批接续：已实现未保存标量/子表列显式选择、路由绑定及后端父/子表权限和行归属检查；子表配置纳入版本摘要。前端54项和真实会话/权限8项通过，未保存子表可编辑用户UI仍待验证。接下来推进SSO及业务HITL，不以当前查询功能代替剩余阶段。

SSO接续：已核对安装版原生OAuth能力并实现平台desk_identity显式映射端点，13项真实身份回归通过。下一步配置原生OAuth客户端/业务回调、会话成员撤销检查及正式Desk入口；尚未宣称浏览器SSO完成，四阶段目标保持完整。

SSO回调更新：原生state/授权码flow/登录适配与加密会话grant、auth_hooks已实现，身份/回调测试通过但交换与登录为替身。下一步先将grant绑定后台运行，再配置OAuth客户端和真实浏览器验收；不把HTTP会话撤权检查当作后台模型执行已覆盖。

后台SSO更新：提交时加密授权已绑定DS Model Run，领取/状态/工具/模型调用/成功结束重查成员；成员版本改变后原生会话轮换。10项会话/SSO/执行组合通过，userinfo仍为替身。继续原生OAuth配置与真实浏览器链，阶段二至四仍未完成。

真实OAuth更新：alpha已配置原生OAuth Client/Social Login Key，真实HTTP授权码→业务Cookie→侧栏会话接口和真实成员撤销通过，SSO组合5项通过。未操作已有浏览器会话。下一步正式平台Desk入口与浏览器点击验收、beta接入，然后业务HITL和后续阶段。

平台入口更新：Portal已替换为企业Desk入口与旧历史只读页面，删除旧前端提交/轮询；47项前端及2项真实入口/映射测试通过。alpha入口已配置，beta与浏览器完整点击仍待验证。旧后端任务提交端点尚未整体退役，阶段二至四继续保持完整范围。

浏览器SSO最新：真实并发测试定位并修复认证的业务→平台→业务循环等待；后续grant检查仅核对平台成员版本，首次登录仍核实绑定。6项SSO及49项前端通过，alpha浏览器平台入口→OAuth→Desk→真实模型/ERP查询→自动展示成功。下一步业务HITL，同时保留beta、并发首次登录、可编辑子表UI与旧后端退役缺口；阶段三/四仍未完成。

再接续：已实现 OS 单写者与 open_runtime，9 项实际 Runtime/锁测试通过；业务 Site claim/tool/finish 与来源再授权已实现，7 项会话/执行回归通过。专用服务身份、HTTP/MCP容器消费及 Desk 接入仍未完成；无新增付费调用。不可将当前能力单测等同真实业务链。

DSH 固定 ref 下：packages/sdk/server/src/index.ts、packages/core/agent/src/runtime-types.ts、docs/subsystems/session.md、packages/session/session-persistence-jsonl/README.md、packages/session/session-checkpoint-policy/README.md、packages/compaction/compaction-basic/README.md、docs/subsystems/skills.md。
Frappe 15.118.0 / ERPNext 15.119.3；实现时读安装源码核验，auth_hooks 才用于最终 token 身份检查，不误用 before_request。首版已认证 HTTP 轮询，不通过 websocket 广播业务正文。

阶段二接续：已落地独立不可变十分钟操作提案与前端逐字段差异/一次确认组件，真实Frappe权限/零业务写入/版本/跨用户2项通过。组件尚未接侧栏，确认执行API和执行记录仍待实现。继续先完成Item/Customer原生save回读去重，再Sales Order及填表；阶段一剩余验收、阶段三/四不缩减。

阶段二最新：确认API、独立执行记录、原生save回读、并发HTTP去重与侧栏提案显示已实现。54项前端、4项真实Frappe通过；真实Item保存验证完成，Customer只验证字段权限，尚未保存。下一步接模型操作领域/固定技能/提案工具，完成真实模型与浏览器HITL，随后Customer创建修改、Sales Order、填表；Unknown主动核实、阶段一缺口、配置预览发布和日常Site仍未完成。

阶段二领域更新：query/operation按消息持久化并隔离原生运行，固定erp-operation技能和只提案不确认的MCP工具已接通；提案绑定生成运行并须其成功退出后才可确认。55项前端、13项Runtime/技能/MCP、12项运行器/压缩、2项真实Runtime→ERP回归、串行14项业务回归及新增来源运行约束5项通过。均未新增付费调用，真实模型→提案→浏览器确认链待下一步执行；完整业务范围和阶段三/四仍保持未完成。

阶段二真实UI更新：alpha普通writer已完成真实deepseek-v4-flash提出Item修改→侧栏一次确认→原生save回读→同页连续真实查询→新标签页历史恢复；草稿未覆盖、执行记录唯一，原预算单次消费者均退出。修复只读页面旧版本的不必要409，保留页面与服务器两个版本；写确认仍严格核对版本。56项前端及13项业务/域名回归通过，详见context-agent-hitl-acceptance.md。下一步直接完成Item/Customer创建修改、Sales Order与填表，并持续推进阶段一缺口和阶段三/四，不以此次Item验收代表全部完成。

阶段二创建更新：Item/Customer原生创建、确认去重、继续修改均通过真实ERP；固定操作skill1.1.0及创建提案MCP已接通。Customer创建另外完成真实模型→浏览器HITL→原生表单回读，只有一个客户/执行，无联系人地址。57项前端、7项真实业务、13项Runtime/技能/MCP通过。接续重点转Sales Order草稿及状态动作、填表；Item创建/Customer修改各自真实模型UI组合仍可补验，不忽略Unknown核实和后续阶段。

阶段二Sales Order服务端更新：普通Sales User真实草稿创建、items修改/新增/移除、分别确认原生提交/取消、重复确认去重通过；与Item/Customer组合8项真实Frappe通过。测试临时订单已清理，原有合成订单未修改。下一步Sales Order读API/子表schema/页面快照、MCP状态提案及skill版本，然后真实模型与UI；当前尚未接通销售订单侧栏。全部持续目标保持进行中，不将该服务端切片当作阶段二或整体完成。

阶段二Sales Order接入更新：读API、可读明细schema/来源列再授权、页面快照、只生成状态提案的MCP已接通；固定query1.1.0/operation1.2.0，侧栏显示中文状态及影响。58项前端、13项Runtime/技能/MCP和19项真实Frappe通过。下一步真实模型Sales Order提案→浏览器确认→原生回读，以及明细可读差异/填表；阶段一缺口、Unknown核实、阶段三/四保留，不缩减目标。

阶段二Sales Order真实链更新：普通writer真实模型→创建草稿→浏览器确认→原生表单回读成功，SAL-ORD-2026-00002；同会话真实明细qty2→3确认后原生总额3703.68且仍草稿。安全错误诊断定位过一次业务HTTP RemoteProtocolError，状态轮询关闭连接复用后真实修改成功，9项运行器/worker通过，既往故障原因仍不能全部宣称已验证。下一步在此合成订单分别真实模型提交/取消与确认，再明细可读差异、填表、剩余阶段一及阶段三/四。不要修改参考订单00001，不将query口头提案当成已形成业务提案。

阶段二Sales Order状态链完成：00002经真实模型提出提交（4次）与取消（3次），分别浏览器确认、原生状态及执行记录回读成功；最终已取消、qty3、总额3703.68，00001未改。首次提交因摘要频繁耗尽预算失败，原生日志定位后TDD调整压缩阈值0.005→0.02，压力摘要与权限拒绝仍验证，14项组合通过。接续重点：表单建议填入/保存分别确认、明细差异可读展示、Unknown核实及剩余验收，再阶段三配置隔离预览发布与阶段四日常Site。目标保持active，不能结束于Sales Order切片。

阶段二填表接入更新：fill标量提案/MCP、Authorized浏览器草稿授权记录、侧栏确认后原生set_value已实现；不保存/刷新，不自动重放，切页/目标草稿变化停止。63项前端、13项Runtime/技能/MCP、7项真实Frappe通过，operation skill1.3.0。下一步真实模型/UI填入和手动保存分别验证，并补目标已提供草稿基线、子表填入和可读明细差异。当前没有真实填入UI验收；所有后续阶段保持未完成。

阶段二标量填入验收：真实模型4次提案→浏览器确认填入→数据库零变更→单独原生保存后回读成功，见HITL记录。已补用户明确提供的form_before标量基线、数据库版本分别检查，65项前端/7项真实ERP/8项Runtime技能通过，operation1.4.0。接续子表填入与可读明细差异、Unknown核实，之后配置预览发布、日常Site和剩余验收；完整目标继续active。

阶段二明细更新：原生逐行set_value填入现有Sales Order行，保留行标识与其他草稿；整表set_value会重建行，未采用。增删重排用业务修改提案；UI按物料/列展示增删改与位置，原始提案仍不可变。68项前端、13项Runtime/MCP、8项真实ERP通过，operation1.5.0，尚未本次真实子表填入UI。下一步真实子表填入/差异验收，再Unknown核实与后续阶段；00002已取消，00001仅可验证未保存草稿，不得保存修改。
# 接续位置（2026-08-29，结果核实切片）

真实模型与原生 UI 子表填入已验证，参考订单00001仅草稿qty5、数据库仍qty2，不得保存该参考单据。Unknown只读核实已实现并通过70项前端、8项真实ERP测试，不重放或改写执行结论。开始阶段三前端配置包/隔离预览/原生发布；阶段一剩余验收、阶段二边界及阶段四仍未完成。具体证据见 context-agent-evidence.md。
# 当前接续：阶段三已开始（2026-08-29）

配置确认卡和数据包解析器已按TDD落地，72项前端、8项包单测及构建通过，尚未接通侧栏/服务端真实预览发布。下一步业务Site配置包持久化、原生权限与基线检查、隔离合成预览及逐项发布，使用原预算不增加常驻服务。不得把解析器或界面组件通过表述为应用发布完成。完整四阶段目标仍active。
# 最新接续：配置包已持久化，预览执行待接通

业务Site DS Configuration Bundle、原生配置权限摘要、配置基线复查、原生配置保存共享连接锁已实现；13项包/真实配置业务回归与10项会话权限执行回归通过。alpha内部审计DocType已加载、backend已重启。下一步原预算隔离合成预览、独立30分钟确认、逐项执行记录、原生保存/工作流/目标发布；尚无预览站点或配置发布，不得跳过这些环节。配置锁跨DDL提交，HTTP请求结束释放，CLI显式finally释放。详情见context-agent-evidence.md，完整目标继续active。
# 当前接续：beta隔离预览入口已运行，应用确认执行待接通

复用既有独立beta合成Site，beta后端已移除外网网络；现有nginx新增preview.localhost:18085，无新增容器/资源限额。预览邮件/调度/实时广播关闭，原生外部连接配置拒绝。5项真实预览/alpha入口/配置回归通过，浏览器登录页已验证。beta Company=0、setup_complete=0；下一步合成配置身份、原生初始化向导，然后独立30分钟预览/发布确认、逐项执行、原生表单保存与工作流。不能将入口可访问当成配置应用成功。其它阶段剩余项与整体目标保持active。
# 最新接续：预览已完成原生初始化

beta合成公司DPR/China/CNY已通过真实原生向导初始化，setup_complete=1、Sales Order=0。合成配置用户dsherp-preview@example.invalid已创建（私有profile），旧beta-reader角色未变；当前新预览浏览器标签由原生向导自动切到了beta-reader。disable_async改为原生Site配置并删除无效扩展钩子，4项真实回归通过。scheduler已停并移入scheduled profile，原配额转beta448MiB/.25CPU，实际总量仍3712MiB/1.9CPU。下一步直接实现30分钟预览/发布确认、逐项执行、原生DocType/字段/工作流应用与回读。Workflow原生保存会补写状态，须防止对已有/并发新记录隐式回填；日常Site阶段重新核对调度资源。完整目标仍active。
# 当前接续：预览原生DocType确认执行已通过

独立30分钟DS Configuration Confirmation和逐项DS Configuration Execution已落地，冻结完整原生文档及权限内容；真实beta原生创建DocType→原生记录insert/save回读→重复确认去重通过。10项确认/应用/包测试与4项隔离/配置回归通过，测试结构与记录已清理。当前尚未接前端、真实模型、跨站预览传递、Workflow或目标发布；workflow包在任何DDL前明确报未接通，下一步必须补完而非删减。还需每步权限复核、部分失败验证及Workflow隐式回填/并发记录保护。完整目标继续active，日常Site与既有验收缺口保留。
# 接续更新：逐步权限复查已验证

真实原生配置创建后禁用合成用户的红测曾复现第二步仍执行；现每步check_authorization，已完成保留Succeeded、未执行步骤Failed、整体Partial。恢复访问后重复确认不补做，5项回归通过，测试身份已恢复且结构清理。接下来实现Workflow（显式状态字段/状态动作/防隐式回填与并发插入）、配置前端和跨站预览/发布。未完成项不缩减，持续目标active。
# 当前接续：Workflow原生预览已跑通

workflow临时拒绝已移除，冻结状态/动作/显式状态字段与原生Workflow；真实新记录草稿→原生工作流提交→取消及回读通过。新自定义表在配置锁被其他连接持有时禁止插入；已有记录故障注入则停止Workflow、不回填、保留Partial且不重跑。6项配置组合、14项回归通过，展示补齐后3项重跑通过，测试结构已清理。下一步配置领域MCP/skill/侧栏接线、跨站预览传递与目标发布；Custom Field、过期/并发确认/未知核实仍需补验。完整四阶段目标active，不将当前服务端Workflow链等同模型/UI/发布完成。

# 最新接续：配置侧栏与原生应用 UI 已验收

74 项前端、8 项真实 ERP 确认/会话回归及构建通过。beta 合成配置操作员在侧栏确认 04ntb13sph，7 个原生配置步骤成功，执行 27u5aot33c 唯一；DS Preview UI Inspection 原生列表、新表单保存、批准、取消已实测，记录 31lbccbdik 最终 docstatus=2。刷新后恢复成功记录、确认禁用、不重放。合成 fixture 不是模型生成，未完成跨站预览或发布。下一步配置领域/MCP/固定 skill/来源运行、跨站预览与独立目标发布，补 Custom Field 和边界验收；阶段四与原有缺口保持，整体 active。

# 当前接续：配置来源运行约束已接入

配置包不可变 payload 绑定当前用户/会话/configuration 领域的来源运行；未成功时展示但不执行，成功后可确认，已有执行不重放。75 项前端及构建、4 项配置组合通过；来源测试移除 rollback 替身后真实事务重跑通过。alpha/beta DS Model Run 新领域 schema 已加载，backend 重启。下一步实际配置读取/提案 MCP、固定业务 skill、领域权限摘要/来源再授权与 UI 选择；这些仍未接通，不将来源约束当成模型链完成。随后跨站预览/独立发布和阶段四，完整目标 active。

# 最新接续：配置 MCP 与固定 Runtime 链已验证

configuration 领域的两个工具、固定 erp-configuration 1.0.0 skill、领域权限摘要、来源字段/历史权限及配置版本重查已实现。真实 Runtime→MCP→HTTP→beta 读取/提案/成功退出通过（模型为本地替身，三次请求，零 DDL）；新旧真实链 4 项通过，最终版本后配置 Runtime/来源/确认 3 项通过。测试临时状态已清理，alpha/beta backend 重启。下一步侧栏显示不可变配置包、开放配置领域与受控跨站预览流转、独立目标发布、真实模型/UI验收；Custom Field、并发/过期/未知结果核实及阶段四仍待完成。整体目标 active。

# 当前接续：侧栏已显示模型配置包

应用配置领域选择、配置包差异展示、预览站点“查看预览确认”与原有明确执行确认已接线；恢复按 bundle_id 去重展示，不自动准备或执行。80 项前端/构建、3 项真实 ERP、1 项真实 Runtime 配置链通过。跨站入口尚未实现，非预览站点的提示不是最终交付；beta worker 路由和确认过期后重新准备 UI 也待补。

下一步直接实现固定源站/隔离预览站之间的不可变配置交接、同一已验证用户对应身份接收、预览执行结果绑定及源站独立发布确认；只传配置数据，不传业务记录或管理员凭证，不开放模型目标/身份参数。完成后跑真实模型和两站 UI，再补 Custom Field/并发/未知结果及阶段四。完整目标保持 active。

# 当前接续：两站不可变配置交接已通过

源站 DS Configuration Transfer、用户绑定 request_id 去重、固定配对 HMAC 请求/响应、beta 幂等接收与回源权限检查已实现。alpha/beta 合成站已私有配对，不增加容器/资源；实际 alpha→beta 内部 HTTP 交接后仅创建预览包和确认，零 DDL，测试状态清理。停用/重启用户不会恢复旧来源授权。

下一步接源站侧栏“发送到隔离预览”和 beta 原生接收页，读取预览执行凭证后在源站单独生成 30 分钟发布确认并逐项执行；真实模型/UI、Custom Field、并发/过期/未知结果及阶段四仍未完成。整体目标 active。

# 当前接续：跨站预览 UI 和真实 Custom Field 已通过

源站交接按钮、beta 原生接收页、当前用户幂等接收和侧栏确认已接线；83 项前端、两站真实回归通过。真实浏览器从 transfer sm0ivtm566 到 beta 预览确认，Item.ds_cross_site_note 唯一步骤成功；原生 Custom Field 表单与 DB 回读 Data/insert_after=item_name/非必填，既有 Item 无值回填，alpha 字段仍不存在。相关合成记录保留作发布验收基线。

下一步读取 beta 唯一成功执行的签名回执，在 alpha 单独生成 30 分钟 publish 确认；用户再确认后逐项应用 alpha、回读 Custom Field 和既有数据，并补重放/部分失败/未知结果。真实模型链和阶段四仍未完成，整体 active。

# 当前接续：签名回执与目标发布闭环已通过

beta 的签名回执会重新核对来源授权、唯一成功预览执行及当前原生配置；alpha 单独生成 30 分钟 publish 确认，并在确认前和逐项发布前重读回执。真实浏览器以当前合成配置用户在 beta 明确预览确认、在 alpha 独立明确发布确认；最终 Custom Field `Item-ds_agent_publish_note` 两站 Succeeded，alpha/beta 既有 Item 均零回填。失败的旧授权包在任何 alpha DDL 前被拒且不重试。确认/交接/应用/HTTP 8 项、前端定向 27 项及构建通过。

按用户要求收敛旧常驻服务：仓库不使用 enqueue/realtime，worker 与两个 websocket 已停止并移入 legacy profile；scheduler 保持 scheduled profile，HTTP 轮询链及平台/业务/预览入口健康。下一步先补发布过期、并发、Unknown/部分成功的组合验收和真实模型生成配置包 UI 链，再进入日常企业 Site、备份与隔离恢复；阶段一/二剩余兼容项同步收敛，完整目标仍 active。

过期 Pending 确认恢复后不会遮住原配置包，用户可明确生成新确认；旧确认继续只读显示“已过期”，不会自动准备或执行。下一步补服务端零 DDL、并发和 Unknown 主动核实。

# 最新接续：真实模型配置发布与边界验证完成

真实 `deepseek-v4-flash` 已从原生 Desk 页面生成新 DocType 配置包，经 alpha→beta 隔离预览、签名回执、alpha 独立确认发布和原生记录保存回读完成；beta 未出现该业务记录。配置过期确认零 DDL 后可明确重提，并发已认证确认只有一个执行/一个原生配置；Partial、Unknown、Failed 增加当前权限下只读逐项核实，绝不重放或改结论。全前端 88 项、配置组合 7 项及构建通过。

阶段三核心真实链已具备，但仍只属于本地合成验证。现在进入阶段四：已按原预算创建未初始化企业资料的独立日常 Site，不添加常驻后端；继续验证零业务数据、原生登录/初始化入口、备份及隔离恢复，并完成日常 Site 的登录和 Agent 运行接线。阶段一、二剩余兼容项仍保留，整体目标 active。

# 最新接续：日常 Site 基础链已验证

独立日常 Site 已安装三个固定 App，原生向导保持未完成，Company/Item/Customer/Sales Order 全为 0；复用现有 backend/nginx，仅新增 18086 入口，无新常驻服务。平台 daily 企业保持 Provisioning 且成员为 0，OAuth 已准备；业务 Site 的内部 Runtime 用户无业务读取角色。context_worker 已按 profile 支持不同 Site，daily 空队列真实运行零模型调用。

轮换本地验证凭证后，已对最终预初始化状态重新执行带文件备份，并真实恢复到一次性隔离 Site；App、初始化状态和零业务数据回读一致后删除临时恢复站点。下一步不再造假填写企业事实：需要用户提供实际公司初始化资料和普通平台成员到普通业务用户的明确映射，之后完成原生向导、真实日常 SSO/Agent UI 验收。此前阶段一/二兼容清单仍需现场收敛，整体目标 active。

# 最新接续：旧平台执行服务已退役

确认无 Queued/Running 旧任务后，旧 submit/claim/tool/finish/heartbeat、worker/task runner/MCP 和本地执行凭证已删除或禁用；DS Agent Task 及 list/get 只读历史保留，独立 `dsherp-studio` Demo 页面保留且未扩展。新上下文 Agent 所需的固定容器预算配置已抽离，不依赖旧 worker。平台/历史 14 项、上下文 Runtime 7 项、全前端 88 项及构建通过。

继续收敛阶段一/二无需企业资料的多标签、断线、取消及边界回归；日常真实初始化和普通成员映射仍等待外部真实信息，不能用合成身份冒充完成。

# 最新接续：全仓回归已通过

旧 websocket 关闭后的过时验收已改为明确验证 `/socket.io` 不可用，Sales Order 来源 fixture 补齐实际工具名；没有为了旧测试恢复服务。全仓 170 项在 426.13 秒内全部通过，前端 88 项及构建也通过。代码和本地合成链已收口到等待真实日常企业资料/普通用户映射的边界；不能把这些测试或 alpha/beta 合成证据称作日常企业真实上线。

# 最新接续：完成度逐项审计

新增 [`context-agent-completion-audit.md`](../../engineering/context-agent-completion-audit.md)，逐条对照四阶段目标、代码、测试、真实 Runtime/模型/ERP/UI 和当前服务。审计确认阶段一至三核心本地链及阶段四空 Site/备份恢复成立；唯一阻止完整目标完成的产品事实是 daily 尚未初始化公司、没有普通平台成员和普通 ERP 用户映射，因而没有该身份下的 SSO/Agent/UI/HITL 证据。保持目标 active，不用合成 alpha/beta 证据替代。

# 最终接续：日常合成企业已完成真实闭环

用户已明确授权使用模拟数据自行初始化。daily 现为原生已初始化的单一合成企业，普通平台成员到普通 ERP 用户映射、平台 OAuth、原生 Item 页面两轮真实模型查询、独立 HITL 修改确认和原生回读均完成。追问曾因没有本轮工具来源被服务端正确拒绝；query skill 1.2.0 经 TDD 固定后真实复测通过，没有放宽服务端来源约束。验收后四件套备份已恢复比对并清理临时 Site。下一步只剩最终全仓/前端回归、证据数字更新、按功能本地提交；这些完成前目标保持 active。

最终全仓 171 项、前端 88 项和生产构建均通过；日常备份恢复、Runtime 退出及服务清单已核实。完成按功能本地提交后，本计划四阶段结束；本地合成可用不等于生产上线。

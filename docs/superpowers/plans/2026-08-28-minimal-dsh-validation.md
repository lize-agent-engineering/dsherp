...
- [ ] 获得项目专用模型凭证、模型与 endpoint、费用上限后，执行上述 CLI 一次；不自动重试失败请求。
- [ ] 记录 SDK/Runtime/内置 Node 版本、退出状态和脱敏事件种类。不得记录密钥、原始会话或授权头。
- [ ] 无凭证时，标记真实模型调用阻塞；本地 SSE 测试只能证明协议、组合和生命周期，不替代真实模型或 ERP。
- [ ] 按功能提交基线文档与最小调用验证器，更新 README 和原计划实际状态；清理不再需要的 work/ 下载。

ERP MCP 工具与租户安全隔离不包含在此零工具验证器中；待隔离站点就绪后独立制定准确接口测试，不以替身数据代替 ERP 验证。

执行记录：17 项测试通过；完整测试含真实 Runtime + 本地 SSE 模型替身。空响应由上游错误结束路径拒绝，不匹配响应由验证器拒绝。真实模型步骤等待凭证与费用授权，未完成；不将本地替身结果替代真实调用。

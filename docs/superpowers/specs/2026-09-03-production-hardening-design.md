...
| 3 | 部署制品与安全边界（2026-09-05 由 Claude 直接实施；G4 在 dev 与生产形态实测通过，G1 按用户指示在本机 Docker 以生产形态整体执行通过，x86_64/ACME/systemd 待真机） | 工作流 A 全部；B 的出口控制、非 root、SSO 强制、guest 加固、CSP | G1、G4（证据：[deployment-security-evidence](../../engineering/deployment-security-evidence.md)，runbook：[deployment-runbook](../../engineering/deployment-runbook.md)） | 2（worker 多站形态确定后再打包） |

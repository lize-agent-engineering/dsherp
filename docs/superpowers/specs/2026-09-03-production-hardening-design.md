...
| 序 | 计划 | 覆盖 | 关键放行门 | 依赖 |
|---|---|---|---|---|
| 1 | 可观测与失败回放（2026-09-03 实施完成，C4 待审计放行） | 工作流 D 全部；F 的评估集导出（只导出，不做断言） | G6；历史失败运行全部导出为评估用例，新失败运行可从事件流回放到工具级（计划：[2026-09-03-observability-replay](../plans/2026-09-03-observability-replay.md)） | 无。先做的理由：后续所有计划的验证都依赖事件流与日志 |

...
def read_schema_all(doctype):
    ...
    Purchase Order 有 105 个字段，一页装不下，所以第一页只是结构的一部分。第一次调用
...
try:
    ...
    supplier_schema,supplier_fields=read_schema_all('Supplier')
    ...
    frappe.set_user('Guest')
    purchase_order_schema,purchase_order_fields=read_schema_all('Purchase Order')
    ...
    assert all('fields' not in field for field in purchase_order_fields.values()
               if field['fieldtype']=='Table'),'默认不应内联任何子表'
    assert purchase_order_fields['items']['rows_of']=='Purchase Order Item',\
        purchase_order_fields['items']
    item_fields=read_child_columns('Purchase Order','items')
    ...
    assert 'items' not in purchase_order_read['fields'],sorted(purchase_order_read['fields'])
    assert purchase_order_read['child_tables']['items']=={
        'child_doctype':'Purchase Order Item','rows':1,
    },purchase_order_read['child_tables']
    assert purchase_order_read['fields']['docstatus']==0,purchase_order_read['fields']['docstatus']
    purchase_order_lines=run_tool(**cap,tool='erp_read_record',arguments={
        'doctype':'Purchase Order','name':purchase_order,'children':['items'],
    })
    assert len(purchase_order_lines['fields']['items'])==1,purchase_order_lines['fields']['items']
    assert purchase_order_lines['fields']['items'][0]['item_code']==item_code
    assert flt(purchase_order_lines['fields']['items'][0]['qty'])==purchase_qty
    assert purchase_order_lines['fields']['items'][0]['warehouse']==warehouse

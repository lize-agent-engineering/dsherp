...
for name in frappe.get_all('DS Configuration Bundle',filters={'conversation':SESSION},pluck='name'):frappe.db.delete('DS Configuration Bundle',{'name':name})
frappe.db.delete('DS Run Event',{'run':RUN});frappe.db.delete('DS Model Run',{'name':RUN});frappe.delete_doc('DS Conversation',SESSION,force=True)

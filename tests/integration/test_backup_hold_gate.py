...
def test_a_held_site_refuses_to_hand_out_a_claim_even_to_the_runtime_identity():
    ...
    claim landing: the server itself refuses while dsherp_hold_until lies ahead."""
    script = r'''
...
try:
    ...
    import time
    frappe.conf.dsherp_hold_until=int(time.time())+300
    ...
    frappe.conf.dsherp_hold_until=int(time.time())-1   # a hold whose end has passed
    task=execution.claim_run('a'*64)
    assert task and task['run_id']==run,'claiming must resume once the hold is lifted'
finally:
    frappe.db.rollback();frappe.conf.dsherp_hold_until=0;frappe.set_user('Administrator')

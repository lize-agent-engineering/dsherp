...
it('源站只交接冻结包并提供预览入口，不在源站执行预览',async()=>{
 const transfer=vi.fn(async()=>({id:'T1',preview_url:'http://preview.localhost/desk/dsherp-configuration-preview/T1'}));
...
it('源站读取预览回执后生成独立发布确认',async()=>{
 ...
 const confirm=vi.fn();
 render(<ConfigurationBundle bundle={{...bundle,preview_available:false,preview_transfer_available:true,transfer:{id:'T1',preview_url:'http://preview.localhost/desk/dsherp-configuration-preview/T1'}}} onPublish={publish} onConfirm={confirm}/>);
